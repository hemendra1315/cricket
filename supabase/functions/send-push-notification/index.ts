...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: announcement, error: annErr } = await supabase
      .from('announcements')
      .select('id, academy_id, title, message, audience, batch_id')
      .eq('id', announcement_id)
      .single();
    if (annErr || !announcement) return new Response('Announcement not found', { status: 404 });

    // 2. Resolve target user IDs based on audience
    let userIds: string[] = [];

    if (announcement.audience === 'batch' && announcement.batch_id) {
      const { data: members } = await supabase
        .from('batch_players')
        .select('academy_member_id, academy_members(user_id)')
        .eq('batch_id', announcement.batch_id);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      userIds = (members ?? []).map((m: any) => m.academy_members?.user_id).filter(Boolean);
    } else {
      ...
      const role = roleFilter[announcement.audience as string] ?? null;
      let query = supabase
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      userIds = (members ?? []).map((m: any) => m.user_id).filter(Boolean);
    }
